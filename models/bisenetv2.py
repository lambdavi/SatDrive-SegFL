...
class ContextPath(nn.Module):
    def __init__(self):
        ...
        self.conv7 = ConvBlock(256, 512, kernel_size=1)
...
class BisenetV2(nn.Module):
    def __init__(self, n_classes, pretrained=False):
        ...
        self.arm1 = AttentionRefinementModule(512, 256)
        self.arm2 = AttentionRefinementModule(256, 128)
        self.conv_gather = ConvBlock(512, 128, kernel_size=3, padding=1)
        self.conv_cls = nn.Conv2d(128, n_classes, kernel_size=1)
        self.conv_up = nn.Conv2d(128, 128, kernel_size=1)
    ...
    def forward(self, x):
        ...
        global_info = torch.mean(context_output, dim=(2, 3), keepdim=True)
        global_info = self.conv_gather(global_info)

        # Attention Refinement Modules
        arm1_out = self.arm1(context_output)
        arm2_out = self.arm2(arm1_out)

        # Upsample
        upsampled = self.up(arm2_out)
        upsampled = self.conv_up(upsampled)

        # Concatenate
        fused = torch.cat((upsampled, global_info), dim=1)

        # Classification
        cls_output = self.conv_cls(fused)

        # Upsample for output
        output = self.up(cls_output)
